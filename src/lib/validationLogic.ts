    ...
    try {
        ...
        const validateNode = (node: any, currentPath: string, currentSchemaDef?: SchemaTypeDefinition) => {
            ...
            Object.entries(node).forEach(([key, value]) => {
                ...
                // Basic check: Is the property known?
                if (!propertyDef && key !== '@context' && key !== '@type') {
                    const message = `Property "${key}" is not a recognized schema.org property.`;
                    result.warnings.push(message);
                    result.errorLocations.push({ startLineNumber: 1, endLineNumber: 1, message: message, severity: 'warning', parentPath: currentPath });
                }

                // Check if property belongs to the current type
                if (currentSchemaDef && propertyDef && !currentSchemaDef.properties.includes(key)) {
                    let isInherited = false; // Basic check, enhance later if needed
                    // TODO: Enhance inheritance check by traversing type hierarchy if necessary
                    if (!isInherited) {
                        const message = `Property "${key}" is not typically expected for type "${currentSchemaDef.id}".`;
                        result.warnings.push(message);
                        result.errorLocations.push({ startLineNumber: 1, endLineNumber: 1, message: message, severity: 'warning', parentPath: currentPath });
                    }
                }
